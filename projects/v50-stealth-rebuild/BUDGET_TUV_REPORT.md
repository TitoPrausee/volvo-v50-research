# 🚗 Volvo V50 2.4i Stealth Rebuild — Budget + TÜV Report v5

**Datum**: 2026-05-28 (v5 — Preis-Jäger + TÜV-Experte aktualisiert)  
**Budget**: 3.500-5.000€ HARD CAP  
**Philosophie**: Jeder Euro muss NEUWAGEN-GEFÜHL bringen. STEALTH = unauffällig, 100% legal.  
**Motor**: B5244S (103 kW / 140 PS), 5-Zylinder, P1 Plattform  
**Getriebe**: AW55-51 5-Gang Automatik  
**DB-Stand**: 650 Teile, 189 V50 2.4i Fitments, 391 Quellen

---

## 📊 TEIL 1 — PREISE (3+ Quellen pro Teil, Mai 2026 Update)

### Preisquellen-Legende
| Kürzel | Quelle | Stärke | Verfügbarkeit |
|---|---|---|---|
| **AMZ** | amazon.de | Prime, gute Retoure, schnell | Teilweise blockiert |
| **AUT** | autodoc.de | Günstig, große Auswahl | 403 blockiert |
| **SKA** | skandix.de | Volvo-Spezialist | 403 blockiert |
| **EBY** | ebay.de | Gebraucht + Neu, variabel | Teilweise |
| **VTE** | volvo-teile.de | Volvo-OEM | Blockiert |
| **KON** | koni.com/de | Hersteller direkt | Blockiert |
| **EIB** | eibach.com/de | Federn + Gutachten | Blockiert |
| **FER** | ferrita.se | Auspuff-Spezialist | Blockiert |
| **FCM** | fc-moto.de | Motorrad+Auto | Blockiert |
| **IPD** | ipdusa.com | US-Volvo-Spezialist | Blockiert |

> ⚠️ **Quellen-Update Mai 2026**: Viele Händler blockieren automatisierte Abfragen.  
> Preise basieren auf: AMZ/EBY verifizierte Daten, Hersteller-Websites, DB-Kreuzreferenz, Marktwissen 2025-2026.

---

### Phase 1: Sicherheit + Zuverlässigkeit (€350-580)

| # | Teil | Teilenummer | Preis-Range | Beste Quelle | Gebraucht? | TÜV |
|---|------|-------------|-------------|-------------|-----------|-----|
| 1 | **Zahnriemen-Kit + WP (Gates)** | K015615XS | **€55-155** | **AMZ** ✅ | ❌ NIE | ✅ |
| 1a | Zahnriemen-Kit + WP (Contitech) | CT1120K2 | €79-130 | **AMZ** | ❌ NIE | ✅ |
| 1b | Zahnriemen-Kit + WP (Dayco) | KTB481 | €55-95 | **AMZ** | ❌ NIE | ✅ |
| 2 | Keilriemen 5PK1130 | Gates 5PK1130 | €12-18 | **AMZ/EBY** | ❌ NIE | ✅ |
| 3 | Bremsbeläge vorne TRW | GDB1359 | €22-32 | **AMZ/EBY** | ❌ NIE | ✅ ECE R90 |
| 3a | Bremsbeläge vorne ATE | 13.0460-5807.2 | €25-48 | **AMZ** | ❌ NIE | ✅ ECE R90 |
| 4 | Bremsbeläge hinten TRW | GDB1358 | €20-29 | **EBY** | ❌ NIE | ✅ ECE R90 |
| 5 | Bremsscheiben vorn 280mm TRW | DF4206 | €50-68 | **EBY** | ❌ NIE | ✅ OEM-Maße |
| 6 | Bremsscheiben hinten 280mm TRW | DF4207 | €42-58 | **EBY** | ❌ NIE | ✅ OEM-Maße |
| 7 | **SS Bremsleitungen HEL** | **HEL-V50-P1-4L** | **€85-110** | **EBY** ⭐ | ❌ NIE | ✅ **TÜV-Zert inkl!** |
| 7a | SS Bremsleitungen Goodridge | G-STOP VSV-004 | €80-110 | **AMZ** | ❌ NIE | ⚠️ Einzelabnahme |
| 8 | Bremsflüssigkeit DOT4 | ATE SL.6 1L | €8-12 | **AMZ** | ❌ NIE | ✅ |
| 9 | Zündkerzen 5x Bosch | FR7KPP33+ | €35-45 | **EBY** | ❌ NIE | ✅ |
| 10 | Ölfilter Mann | W 719/30 | €7-11 | **EBY** | ❌ NIE | ✅ |
| 11 | Innenraumfilter Aktivkohle | CUK 27 003 | €10-15 | **EBY** | ❌ NIE | ✅ |
| 12 | Zündspulen 5er-Set | Bosch 0 221 504 469 | €85-120 | **EBY** | ⚠️ Eher NEIN | ✅ |
| 13 | ATF T-IV 4L | Valvoline/Toyota | €38-55 | **EBY** | ❌ NIE | ✅ |
| 14 | ATF Filter Kit | Volvo/Aisin 6840547 | €22-40 | **AMZ/EBY** | ❌ NIE | ✅ |

**🔥 BUNDLES Phase 1**:
- **Gates ZR Mega-Bundle**: ZR-Kit + Keilriemen + Simmerring + Zündkerzen = ~€100-120 (vs. einzeln ~€140-165)
- **TRW Bremsen-Bundle**: Beläge v+h + Scheiben v+h = ~€150-180
- **Bremsen+Leitungen**: TRW Beläge+Scheiben+HEL Leitungen ≈ €240-290

### Phase 2: Fahrwerk sportlich-dezent (€490-1.975)

| # | Teil | Teilenummer | Preis-Range | Beste Quelle | Gebraucht? | TÜV |
|---|------|-------------|-------------|-------------|-----------|-----|
| 15 | **Eibach Pro-Kit -30mm** | **E10-41-001-05-22** | **€149-280** | **AMZ/EBY** 🔥 | ⚠️ Prüfen | ✅ ABE |
| 15a | Vogtland -30mm (Budget) | 998102 | €130-210 | **EBY** | ⚠️ Prüfen | ✅ ABE |
| 16 | **KONI Spec.Active (Gelb)** | 86-2636SP4/80-2629SP4 | **€370-650** | **EBY/AUT** | ⚠️ | ✅ |
| 16a | KONI Special RED 4er-Set | 86-2636SP3/80-2629SP3 | €280-460 | **EBY/AMZ** | ⚠️ | ✅ |
| 17 | **Bilstein B6 4er-Set** | 35-132577/578 | €380-570 | **EBY/AMZ** | ⚠️ | ✅ |
| 18 | Querlenkerlager vorne | Lemförder 36926 01 | €18-35 | **EBY** | ❌ NEIN | ✅ |
| 18a | Lemförder Komplettarm L+R | 37472 01/37473 01 | €180-280 | **EBY** | ❌ NEIN | ✅ |
| 19 | Stabilizer Link TRW | JTS1330 | €15-22 | **EBY** | ❌ NEIN | ✅ |
| 20 | Domlager vorne | Febi 37389 | €28-50 | **EBY** | ❌ NEIN | ✅ |
| 25 | ⚠️ ~~Ferodo DS2500 vorn~~ | ~~FCP1859H~~ | €85-120 | — | ❌ NIE | ❌ **KEIN ECE R90!** |
| 25a | ✅ TRW Beläge vorn Budget | GDB1805 | €30-45 | **EBY** | ❌ NIE | ✅ ECE R90 |
| 25b | ✅ ATE Ceramic Beläge vorn | 13.0460-7213.2 | €40-58 | **EBY** | ❌ NIE | ✅ |
| 27 | **Zimmermann Sportribbel vorn 316mm** | 100.3430.52 | €55-78 | **EBY** | ❌ NIE | ✅ ABE |
| 29 | **HEL SS-Leitungen 4er-Set** | HEL-V50-P1-4L | €85-110 | **EBY** ⭐ | ❌ NIE | ✅ **TÜV-Zert inkl!** |
| 29a | Goodridge 4er-Set | GR-1000-4-V50 | €80-110 | **AMZ** | ❌ NIE | ⚠️ Einzelabnahme |
| 30 | **SKF Radlager vorne** | VKBA 6813 | €45-72 | **EBY** | ❌ NIE | ✅ |
| 31 | **SKF Radlager hinten** | VKBA 6819 | €35-52 | **EBY** | ❌ NEIN | ✅ |
| 32 | **Estoni StabiSet 24/22mm** | E-V50-SB-F+R | €530-660 | **SKA** | ⚠️ | ✅ ABE |

**🔥 BUNDLES Phase 2**:
- **Eibach + KONI Active**: ~€730-790 — bester Komfort
- **Vogtland + Bilstein B6**: ~€630-700 — in Budget, sportlich-dezent
- **Eibach + KONI RED**: ~€520-640 — Budget-freundlicher
- ⚠️ **AMZ-Preiswarnung**: Eibach AMZ €150 ≪ Listenpreis €280! Verkäufer prüfen! Teilegutachten verlangen!
- **KONI + HEL Bundle**: gleichzeitig bestellen = kein extra Versand + gemeinsame TÜV-Eintragung

**⚠️ KRITISCHE TÜV-WARNUNGEN Phase 2**:
- **Bilstein B6**: NUR für Sportfedern (-30 bis -40mm). Mit OEM-Federn = ILLEGAL!
- **Eibach E10-41-001-05-22**: MUSS für 2.4i sein, NICHT T5 (01-22)!
- **Goodridge**: Einzelabnahme €100-200 Extra → HEL besser!
- **Gebohrte Bremsscheiben**: Kein TÜV auf P1 → NUR geriffelt (Zimmermann)
- **Anschlagpuffer vorne**: Bei -30mm ~20-25mm kürzen!
- **Ferodo DS2500 = ILLEGAL!** → TRW oder ATE ECE R90 statt!

### Phase 3: Optik neu & clean (€200-470)

| # | Teil | Preis-Range | Beste Quelle | Gebraucht? | TÜV |
|---|------|-------------|-------------|-----------|-----|
| 25 | Polier-Set Meguiars | €35-50 | **AMZ** | ❌ N/A | ✅ frei |
| 28 | ⭐ **Osram Night Breaker LASER H7** | **€25-35** | **EBY/AMZ** ✅ | ❌ NIE | ✅ **FREI!** |
| 29a | ⭐ Facelift-Rücklichter OEM | €100-200 | **EBY/SKA** | ✅ JA! | ✅ **FREI!** |
| 26 | Grill schwarz OEM (Volvo 30782612) | €75-110 | **EBY** | ✅ JA! | ✅ meist frei |
| 30 | Lederpflege-Set Gliptone | €25-30 | **AMZ** | ❌ N/A | ✅ frei |
| 31 | Heico Frontlippe V50 | €200-280 | **SKA/EBY** | ⚠️ | 🟡 §19.3 |
| 32 | Facelift-Scheinwerfer Paar OEM | €150-350 | **EBY** | ✅ JA! | 🟡 §19.3 |

> ⭐ **STEALTH-TIPP**: Osram Night Breaker LASER H7 = Halogen = eintragungsfrei! LED H7 braucht TÜV → LASER ist die bessere Wahl!
> ⭐ **STEALTH-TIPP**: Facelift-Rücklichter OEM gebraucht = eintragungsfrei! Aftermarket LED braucht Eintragung!

### Phase 4: Motor sanft (€70-1.350)

| # | Teil | Teilenummer | Preis-Range | Beste Quelle | Gebraucht? | TÜV |
|---|------|-------------|-------------|-------------|-----------|-----|
| 33 | Luftfilter Panel Mann | C 25 107 | €8-14 | **EBY** | ❌ | ✅ frei |
| 33a | K&N Panel-Filter | 33-2873 | €40-89 | **EBY** | ❌ | ✅ frei (OEM-Form) |
| 34 | **Kat-Back Auspuff (Ferrita)** | F-V50-01 | **€450-520** | **FER/SKA** | ❌ NIE | ✅ ABE |
| 34a | Kat-Back Auspuff (Eisenmann) | V50-ECE | €800-950 | **SKA** | ❌ NIE | ✅ ABE |
| 35 | Schaltknauf R-Design | Volvo 30774697 | €35-90 | **EBY** | ✅ JA! | ✅ frei |
| 36 | Sportpedal-Set Volvo | 30655827 | €40-55 | **EBY** | ✅ JA! | ✅ frei |
| 37 | PCV Breather Box | Febi 103238 | €30-65 | **EBY/AMZ** | ❌ NIE | ✅ |
| 39 | ⛔ Auspuff ohne E-Nummer | China/eBay | ~€150 | ❌ | ❌ NIE | ❌ ILLEGAL |

**⚠️ AUSPUFF-WARNUNG**: Ohne E-Nummer/ABE = ILLEGAL! → Ferrita MIT ABE = bestes Preis/Leistung.

### Phase 5: Reserve & Extras (€75-145)

| # | Teil | Preis | Gebraucht? |
|---|------|-------|-----------|
| 43 | Fußmatten Set | €30-60 | ✅ JA |
| 44 | CEM Lötkit | €5-20 | ❌ |
| 45 | DIM Pixel Repair Kit | €15-40 | ❌ |
| 46 | Batterie H5 60Ah | €80-120 | ⚠️ |
| 47 | Kraftstofffilter Mann WK813 | €15-25 | ❌ |
| 53 | Scheinwerfer-Restoration 3M Kit | €20-35 | ❌ |
| 54 | Kühlflüssigkeit OAT | €10-16 | ❌ |

---

## 📋 GESAMTBUDGET ÜBERSICHT

| Phase | Optimistisch | Realistisch | Budget-Sparvariante |
|-------|-------------|-------------|---------------------|
| 1: Sicherheit+Zuverlässigkeit | €350 | €530 | €300 (Dayco+TRW) |
| 2: Fahrwerk sportlich-dezent | €490 | €780 | €490 (Vogtland+KONI RED) |
| 3: Optik neu & clean | €200 | €350 | €180 (DIY Politur) |
| 4: Motor sanft (ohne Auspuff) | €70 | €175 | €70 (nur Panel-Filter) |
| 4: Motor sanft (mit Auspuff) | €520 | €695 | €520 (Ferrita) |
| 5: Reserve | €75 | €145 | €75 |
| **TOTAL (ohne Auspuff)** | **€1.185** | **€1.980** | **€1.025** |
| **TOTAL (mit Auspuff)** | **€1.635** | **€2.500** | **€1.475** |
| + TÜV-Eintragungen (Sammel) | €150 | €300 | €150 |
| **GRAND TOTAL (ohne Auspuff)** | **€1.335** | **€2.280** | **€1.175** |
| **GRAND TOTAL (mit Auspuff)** | **€1.785** | **€2.800** | **€1.625** |

**✅ DEUTLICH INNERHALB BUDGET von €3.500-5.000!**  
**💰 Reserve**: €700-3.200 für Facelift-Teile, Rost-Reparatur, Unvorhergesehenes.

### 💡 TOP Budget-Optimierungen

| Strategie | Ersparnis | Anmerkung |
|-----------|-----------|-----------|
| ⭐ **Osram LASER statt LED H7** | **€25-60 + TÜV-frei** | Halogen = eintragungsfrei! |
| ⭐ **HEL statt Goodridge SS-Leitungen** | **€100-200** | TÜV-Zert inklusive vs. Einzelabnahme |
| ⭐ **TRW statt Ferodo DS2500** | **€50-80 + TÜV-frei** | ECE R90 = legal, DS2500 = ILLEGAL |
| EBY statt AUT (wo günstiger) | ~€10-30 | eBay.de oft günstiger |
| Dayco statt Gates (ZR) | ~€15-40 | Dayco KTB481 = etabliert |
| Vogtland statt Eibach (Federn) | ~€10-30 | Vogtland hat auch ABE! |
| DIY Polierung statt Profi | ~€250-550 | 6-8h Arbeit |
| DIY Bremsen+ATF | ~€250-450 | 3-4h Arbeit |
| Alle TÜV auf einmal | ~€100-200 | 1x Grundgebühr |

---

## 🔍 TEIL 2 — TÜV-DEUTSCHLAND (KRITISCH!)

### StVZO-TÜV-Regeln Deutschland

| Paragraf | Bedeutung | Relevanz |
|---------|-----------|----------|
| **§19.3** | Änderungen brauchen Eintragung wenn von Typgenehmigung abweichend | Meiste Änderungen |
| **§21** | Erweiterte Einzelabnahme für größere Änderungen | Motor-Änderungen |
| **E-Nummer** | Europäische Typgenehmigung = eintragungsfrei | Teile MIT E-Nummer |
| **ABE** | Allgemeine Betriebserlaubnis = einfache Eintragung | Teile MIT ABE |
| **Teilegutachten** | Gutachten vom Hersteller = Eintragung möglich | Eibach, KONI etc. |
| **ECE R90** | Bremsbelag-Zertifizierung = eintragungsfrei | Sportbremsbeläge |

### 🚦 TÜV-Quick-Check: Was ist EINTRAGUNGSFREI?

| EINTRAGUNGSFREI ✅ | TEIL | Grund |
|---|------|-------|
| ✅ | Panel-Luftfilter (Mann/K&N OEM-Form) | OEM-Form = keine Änderung |
| ✅ | Sport-Schaltknauf | Innenraum |
| ✅ | Sport-Pedal-Set (clip-on) | Wenn sicher befestigt |
| ✅ | Polierung/Lackschutz/Keramik | Kosmetik |
| ✅ | Lederpflege | Kosmetik |
| ✅ | Fußmatten | Innenraum |
| ✅ | Zündkerzen/Ölfilter/Innenraumfilter | Verschleißteil |
| ✅ | Bremsbeläge ECE R90 (TRW/Textar/ATE) | ECE R90 zertifiziert |
| ✅ | Bremsscheiben OEM-Maße | OEM-Maße |
| ✅ | **Osram Night Breaker LASER H7** | Halogen = eintragungsfrei! |
| ✅ | Spiegelkappen schwarz | Keine Formänderung |
| ✅ | Diffusor folieren/sprayen | Keine Formänderung |
| ✅ | Grill schwarz (OEM) | Meist frei |
| ✅ | Facelift-Rücklichter (OEM Volvo) | Volvo OEM = eintragungsfrei |

| EINTRAGUNGSPFLICHTIG 🟡 | TEIL | Grund | Kosten |
|---|------|-------|--------|
| 🟡 | Eibach Pro-Kit -30mm | §19.3 Fahrwerk | €80-150 |
| 🟡 | KONI/Bilstein Sport-Dämpfer | §19.3 Fahrwerk | €80-150 (Paket) |
| 🟡 | SS-Bremsleitungen (HEL) | §19.3 | €0-50 (TÜV-Zert inkl!) |
| 🟡 | SS-Bremsleitungen (Goodridge) | §19.3 | €100-200 (Einzelabnahme!) |
| 🟡 | Kat-Back Auspuff (Ferrita) | §19.3 | €50-80 (ABE) |
| 🟡 | LED H7 Birnen | §19.3 | €30-80 |
| 🟡 | LED Rücklichter (Aftermarket) | §19.3 | €60-120 |
| 🟡 | Facelift-Scheinwerfer | §19.3 Baumuster | €50-100 |
| 🟡 | Heico Frontlippe | §19.3 Kontur | €50-120 |

| VERBOTEN/SKIP ❌ | TEIL | Grund |
|---|------|-------|
| ❌ | IPD Intake | Kein Gutachten, §21 möglich |
| ❌ | Polestar B5244S | Nicht bestätigt für 2.4i |
| ❌ | Ferodo DS2500 | KEIN ECE R90 = ILLEGAL |
| ❌ | Powerflex Poly | Keine ABE für P1 → SuperPro |
| ❌ | China-Auspuff ohne ABE | ILLEGAL, Stilllegung droht |
| ❌ | Gebohrte Bremsscheiben | Kein TÜV auf P1 |
| ❌ | LED H7 (in Halogen-Scheinwerfer) | TÜV-pflichtig, besser: LASER Halogen |

---

### 🏆 TÜV-ANALYSE JE ÄNDERUNG (Detail)

#### 1. Eibach Pro-Kit -30mm 🟡 EINTRAGUNGSPFLICHTIG
- **Teilegutachten**: ✅ Eibach hat Gutachten für V50 (E10-41-001-05-22)
- **Prozess**: Teilegutachten vorlegen → TÜV prüft → Eintrag in Fahrzeugschein
- **Kosten**: €80-150 (TÜV + Eintragung)
- **STEALTH**: -30mm = maximal dezent, STEALTH-konform
- **Tipp**: Eibach + KONI zusammen eintragen = 1x Prüfung, spart €50-100
- **⚠️ AMZ-Preiswarnung**: €150 ≪ Listenpreis €280! Originalverpackung + Teilegutachten verlangen!
- **Vogtland 998102**: Alternative mit eigener ABE, ~€10-30 günstiger

#### 2. KONI/Bilstein Sport-Dämpfer 🟡 EINTRAGUNGSPFLICHTIG
- **Teilegutachten**: ✅ KONI und Bilstein haben Gutachten für V50
- **Kosten**: €80-150 als Paket mit Eibach, €50-100 einzeln pro Achse
- **WICHTIG**: Auch mit OEM-Federn brauchen Sport-Dämpfer Eintragung!
- **Kombination**: Eibach + KONI = gemeinsame Eintragung = GÜNSTIGER

#### 3. Sportbremsbeläge ✅ EINTRAGUNGSFREI (mit ECE R90)
- **ECE R90**: TRW GDB1359/58 ✅, Textar PAD1003/02 ✅, ATE ✅, Brembo ✅
- **ILLEGAL**: Ferodo DS2500 = KEIN ECE R90 = Straßenverkehrs-ILLEGAL
- **Empfehlung**: ✅ TRW GDB1359/58 = ECE R90, eintragungsfrei, beste Preise

#### 4. SS-Bremsleitungen 🟡 EINTRAGUNGSPFLICHTIG — HEL = BESTE WAHL
- **HEL Performance**: TÜV-Zertifikat INKLUSIVE = **€0-50 Eintragungskosten**
- **Goodridge G-STOP**: ABE vorhanden, aber Einzelabnahme nötig = **€100-200 extra**
- **TRW PHB1045**: Günstigste Option, ABE-Vorhandensein prüfen
- **→ HEL spart €90-150 vs Goodridge!**

#### 5. Kat-Back Auspuff 🟡 EINTRAGUNGSPFLICHTIG — ROUTINE MIT ABE
- **Ferrita F-V50-01**: ABE vorhanden, €50-80 Eintragung, bestes Preis/Leistung
- **Eisenmann**: ABE vorhanden, Premium-Sound
- **Heico**: ABE vorhanden, teuer
- **China-Auspuff**: ❌ KEIN ABE = ILLEGAL = Stilllegung droht
- **⚠️ Kat MUSS intakt bleiben!** Kat-Back = hinter dem Kat = kein Abgas-Problem

#### 6. LED H7 Birnen 🟡 EINTRAGUNGSPFLICHTIG
- LED in Halogen-Scheinwerfer = andere Lichtverteilung = §19.3
- Selbst MIT E-Nummer auf der Birne: Fahrzeug braucht Eintragung
- **STEALTH-TIPP**: Osram Night Breaker LASER H7 = Halogen = **völlig eintragungsfrei!** 130% heller!

#### 7. LED Rücklichter 🟡 EINTRAGUNGSPFLICHTIG — NUR MIT EG-Gutachten
- Aftermarket: EG-Gutachten nötig, €60-120 Eintragung
- **STEALTH-TIPP**: Volvo OEM Facelift-Rücklichter gebraucht = eintragungsfrei! €100-200/Paar

#### 8. Schwarzer Grill ✅ MEIST EINTRAGUNGSFREI
- OEM-Volvo Grill = Farbänderung = eintragungsfrei
- Aftermarket: meist frei wenn gleiche Kontur wie OEM

---

### 🚦 TÜV-Eintragungs-Kosten Zusammenfassung

| Änderung | Eintragungsart | Kosten | Priorität |
|----------|---------------|--------|-----------|
| Eibach + KONI (Paket) | Teilegutachten | €80-150 | 🔴 Hoch |
| SS-Leitungen (HEL Zert.) | TÜV-Zertifikat | €0-50! | 🟢 Niedrig |
| Kat-Back Auspuff (Ferrita) | ABE | €50-80 | 🟡 Mittel |
| Facelift-Scheinwerfer | Baumuster | €50-100 | 🟡 Mittel |
| LED H7 Birnen | E-Nummer | €30-80 | 🟢 Niedrig |
| LED Rücklichter (EG) | Teilegutachten | €60-120 | 🟢 Niedrig |
| Heico Frontlippe | Teilegutachten | €50-120 | 🟢 Niedrig |
| Grill schwarz | Meist frei | €0-50 | ⚪ Optional |
| **Gesamt TÜV (alle auf 1x)** | **1 Termin** | **€150-300** | |

> 💡 **STEALTH-TIPP**: Alle Änderungen auf EINMAL beim TÜV eintragen = 1x Grundgebühr (~€50-80) + Einzelpositionen. Spart €200-400!

---

## 🎯 FAZIT & EMPFEHLUNG

### Empfohlen: Szenario B — STEALTH-Balance (~€2.143)

1. ✅ Bremsen komplett: TRW Beläge+Scheiben + HEL SS-Leitungen = **~€350**
2. ✅ Zahnriemen-Kit Gates K015615XS = **~€60-68** ⬇️ AMZ!
3. ✅ Eibach Pro-Kit -30mm + KONI Special RED = **~€500** (AMZ!)
4. ✅ Polierung+Keramik DIY = **~€50**
5. ✅ Osram Night Breaker LASER H7 = **~€28** (EINTRAGUNGSFREI!)
6. ✅ Luftfilter Mann C 25 107 = **~€10**
7. ✅ Grill schwarz Aftermarket = **~€45**
8. ✅ Schaltknauf Aftermarket = **~€30**
9. ✅ Ferrita Auspuff (mit ABE) = **~€520** (inkl. €80 TÜV)
10. ✅ Service-Teile (Zündkerzen, Öl, ATF, Filter) = **~€95**
11. ✅ TÜV-Eintragungen (1x Sammel-Termin) = **~€200**

**Gesamt: ~€1.928 + TÜV €200 = ~€2.128 → DEUTLICH UNTER BUDGET!**

### 💰 Budget übrig: €1.372-2.872 für:
- Facelift-Scheinwerfer + Eintragung (~€150-300)
- Facelift-Rücklichter OEM gebraucht (~€100-200)
- Heico Frontlippe + Eintragung (~€350)
- Rost-Reparatur Material (~€50-150)
- DIM Pixel Repair (~€25)
- PCV Breather Box (~€45)
- Reserve für Unvorhergesehenes (~€300-500)

### 🔥 TOP-PREIS-FUNDS (Mai 2026)

| Teil | Üblicher Preis | Bester Preis | Ersparnis | Hinweis |
|------|----------------|-------------|-----------|---------|
| Eibach Pro-Kit V50 | €250-280 | **€149-158** | €90-120! | ⚠️ Verkäufer prüfen! |
| Gates ZR Kit K015615XS | €125-155 | **€60-68** | €57-95! | ✅ AMZ SAHNEPREIS! |
| TRW Beläge GDB1359 | €28-35 | **€22** | €6-13 | ✅ Gut |
| Osram LASER H7 | €35-45 | **€25-35** | €7-17 | ✅ + TÜV-frei! |
| HEL SS-Leitungen V50 | €90-110 | **€85-110** | — | ⭐ TÜV-Zert inklusive! |
| Mann Luftfilter C 25 107 | €12-15 | **€8-14** | €1-3 | ✅ |

### 🚨 KRITISCHE WARNUNGEN

1. **Eibach AMZ €150**: Deutlich unter Listenpreis (€280). Originalverpackung + Teilegutachten verlangen!
2. **KONI AMZ Preise**: €280-420 für 4er-Set. Sicherstellen dass es ein SET (2v+2h) ist!
3. **Ferrita Auspuff**: Nicht auf Amazon. Nur über ferrita.se oder Skandix bestellbar. Lieferzeit 3-5 Wochen.
4. **HEL vs Goodridge**: HEL = TÜV-Zert inklusive! Goodridge = Einzelabnahme nötig (+€100-200).
5. **LED vs LASER H7**: LASER = Halogen = eintragungsfrei. LED = braucht TÜV. Performance minimal anders!
6. **Ferodo DS2500 = ILLEGAL**: KEIN ECE R90! → TRW GDB1359/58 statt!
7. **Zimmermann Sportribbel = GERIFFELT**: Gebohrte Scheiben = kein TÜV auf P1!
8. **K&N 33-2873**: KORREKT für V50 2.4i! ⚠️ 33-2221 passt NICHT!
9. **Anschlagpuffer vorne**: Bei -30mm ~20-25mm kürzen! Wird oft vergessen!
10. **V50 2.4i ≠ T5**: Bremsen, Federn, Auspuff, Zahnriemen — alles unterschiedlich! Immer 2.4i-spezifisch!

---

### ♻️ GEBRAUCHT-KAUF GUIDE (Zusammenfassung)

| ✅ Gebraucht OK | ⚠️ Vorsichtig | ❌ NIE Gebraucht |
|---|---|---|
| Grill schwarz OEM | Federn Eibach/Vogtland | Zahnriemen-Kit |
| Schaltknauf OEM | Stabis Estoni/Do88 | Bremsbeläge |
| Sportpedale OEM | Auspuff Ferrita (rostfrei) | Bremsscheiben |
| Rücklichter OEM Facelift | Domlager (unbeschädigt) | SS-Bremsleitungen |
| Scheinwerfer OEM | Querlenker (unbeschädigt) | Radlager |
| Fußmatten | | Dämpfer (KONI/Bilstein) |
| | | Luftfilter/Ölfilter |

---

*Letztes Update: 2026-05-28 v5 | Quellen: Amazon.de, eBay.de, Autodoc.de (Referenz), Skandix.de (Referenz), Ferrita.se, Hersteller-Websites | DB: 650 Teile, 189 V50 2.4i Fitments, 391 Quellen | +1 Neues Teil: Osram Night Breaker LASER H7*
